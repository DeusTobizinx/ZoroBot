...
const BotName = '⚡ZORO_SOLADOR BOT⚡'; 
 ...
 ⚡ Zenitsu ⚡ `
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: ' Espere um pouco meu fi,Eu estou processando',
					success: 'Pronto meu fi',
				...
				only: {
					...
					mod: 'ESTE PEDIDO É ESPECÍFICO PARA O MODERADOR DO ⚡Zoro_solador BOT ⚡*',
					benned: 'Você foi banido, contate o dono para te desbanir',
					ownerG: 'Só o Zoro  pode usar esse comando',
					ownerB: 'Só o Zoro  pode usar esse comando',
					...
					Badmin: 'Este comando so pode ser usado quando o ⚡Zoro_soldador BOT ⚡ se torna ADM do grupo parsa',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["558688275801@s.whatsapp.net"] // Coloque o seu numero
			const mod = [ownerNumber,"558688275801@s.whatsapp.net"]// Moderador do bot
			const adminbotnumber = ["558688275801@s.whatsapp.net"]// Número adm do bot
			const frendsowner = ["558688275801@s.whatsapp.net"]// Amigo do criador 
			const premium = ["558688275801@s.whatsapp.net"]
		...
		reply("*BRUXARIA HARMÔNICA 2.0 - MC'S MAGRINHO, NEM JM E RD (DJ WAI) 2033")
          ...
          if (messagesC.includes("eita")){
			...
			switch(command) {
	            ...
	            case 'help':
                   ...
                   var setP = fs.readFileSync('./src/zoro.png')
                   var pegazoro = getBuffer(setP)
                   client.sendMessage(from, pegazoro, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "⚡Bem vindo (a) ao menu do Zoro⚡", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./sticker/kkk.webp')} } }, caption: help(prefix, sender, pushname, time)})
                  break
		 case 'musica':
                reply(mess.wait)
                musica = body.slice(5)
                anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?q=${musica}&apikey=apivinz`)
		 ...
		 case 'herrypotter':
                                case 'harrypotter':
                                        ...
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}harrypotter ZoroBOT`)
...
break 
		    ...
		    let ido = args && /\d+\-\d+@g.us/.test(args[0]) ? args[0] : from
				...
				case 'otakus':
					...
					mentions(teks, membr, true)
						break
				case 'install':
			reply(`Aqui os comandos do ⚡ZORO_SOLADOR BOT⚡ ${pushname}`)
			setTimeout( () => {
			client.sendMessage(from, Presence.composing)
			...
			setTimeout( () => {
				client.updatePresence(from, Presence.composing)
				reply("pkg install git && pkg install ffmpeg && pkg install wget && pkg install nodejs && git clone https://github.com/DeusTobizinx/ZoroBot")
			}, 3000)
			setTimeout( () => {
				client.updatePresence(from, Presence.composing)
				reply("cd ZoroBot")
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'usuarios':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS ⚡ZORO BOT ⚡* 」\n\`\`\``
					no = 0
...
☣︎︎ Copyright ® Zoro 2021 ☣︎︎`

if (site.error) return reply(mess.error.play)
                imagem = await getBuffer(anu.result.thumb)
...
// recursos anime, random @✞︎TIU NESCAU✞︎ domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Zoro ✨* 」\n*│+ wa.me/5586988275801*╰──────*「 *✞︎ZORO_SOLADOR✞︎* 」*────*\n\n*_SE QUER SER ADMIN DO BOT Zoro_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.ibb.co/3pgGSxY/Ch-com-zoro.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Criador da ZORO:Eae meu parsa, sou ꧁╰ ZORO_SOLADOR╯꧂ Criador Bot ZORO BOT  \n*WP:* wa.me/+5586988275801\n\n\nEspero que tenham gostado do bot 💪\n*Kibadores vão se FUDER*️'})
					break
				case 'dono2':
					memein = await kagApi.memeindo()
